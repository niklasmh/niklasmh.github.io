...
class Me extends Component {
  constructor() {
    ...
    if (
      ...
      timeNow - thresholdTime > lastUpdate
    ) {
      API.getRequest(
        'https://api.github.com/users/niklasmh/repos',
        repos => {
    ...
    const requests = repos
      .map(async repo => {
        let response = await fetch(
          `https://api.github.com/repos/niklasmh/${
            repo.name
          }/stats/contributors`
        )
        ...
        if (contribs instanceof Array) {
          repoData[repo.name] = contribs.filter(
            stat => stat.author.login === 'niklasmh'
          )[0]
        } else {
      ...
      .concat(
        repos.map(async repo => {
          ...
          let response = await fetch(
            `https://api.github.com/repos/niklasmh/${repo.name}/topics`,
            req
          )
  ...
  render() {
    ...
    let reposLeft =
      this.state.repos.length - Object.keys(this.state.repoData).length
          ...
          <SubSection flex="1 1 320px" className="recent-project-container">
            ...
            {!!Object.keys(this.state.repoData).length ? (
              !!reposLeft && (
                <div
                  className="load-more"
                  onClick={() => this.fetchRepos(this.state.repos, reposLeft)}
                >
                  {/*Load more... ({reposLeft})*/}
                </div>
              )
